...
class CheckResultIntegrationTests(@Autowired val mockMvc: MockMvc) : BaseTestWithReusingContainers() {
    ...
    fun `test if secured by permissions teamId`() {
        mockMvc.get("/v1/check-result?teamId=4Lxhu5YKWPBr&page=0&size=10").andExpect {
            status { isForbidden() }
        }
    }
    ...
    fun `test if secured by permissions monitorId`() {
        mockMvc.get("/v1/check-result?monitorId=k6A6bEK7C9pC&page=0&size=10").andExpect {
            status { isForbidden() }
        }
    }
    ...
    fun `test if accessible with user`() {
        mockMvc.get("/v1/check-result?page=0&size=10").andExpect {
            status { isOk() }
            content {
