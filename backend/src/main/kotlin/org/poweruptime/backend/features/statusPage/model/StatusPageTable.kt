...
object StatusPageTable : ULongIdTable("status_page"), HasPublicId, HasModifiers, HasSoftDelete, HasName {
    ...
    val imageId = ulong("image_id").references(FileTable.id).nullable()
...
fun StatusPageTable.rowToStatusPageRecord(row: ResultRow): StatusPageRecord =
    StatusPageRecord(
        id = row[id].value,
        ...
        teamId = row[teamId],
        imageId = row[imageId],
        image = if (row[imageId] != null) FileTable.rowToFileRecord(row) else null,
