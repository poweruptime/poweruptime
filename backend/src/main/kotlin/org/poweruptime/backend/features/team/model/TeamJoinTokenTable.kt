...
object TeamJoinTokenTable : NanoIdTable("team_join_token", NANO_ID_MAX_LENGTH), HasModifiers {
    override val createdAt = createdAt()
    override val updatedAt = updatedAt()

    val inviteeId = ulong("invitee_id").references(UserTable.id).index()
    val inviterId = ulong("inviter_id").references(UserTable.id).index()
...
fun TeamJoinTokenTable.rowToTeamJoinTokenRecord(row: ResultRow): TeamJoinTokenRecord =
    TeamJoinTokenRecord(
        id = row[id].value,
