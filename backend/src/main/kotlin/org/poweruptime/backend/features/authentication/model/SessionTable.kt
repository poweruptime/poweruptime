...
object SessionTable : ULongIdTable("session"), HasPublicId, HasModifiers {
...
fun SessionTable.rowToSessionRecord(row: ResultRow): SessionRecord =
    SessionRecord(
        id = row[id].value,
...
object RefreshTokenTable : ULongIdTable("refresh_token"), HasModifiers {
    override val createdAt = createdAt()
    override val updatedAt = updatedAt()

    val sessionId = ulong("session_id").references(SessionTable.id).index()
...
fun RefreshTokenTable.rowToRefreshTokenRecord(row: ResultRow): RefreshTokenRecord =
    RefreshTokenRecord(
        id = row[id].value,
        createdAt = row[createdAt],
        updatedAt = row[updatedAt],
