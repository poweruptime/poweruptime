...
object SubNotificationTable : ULongIdTable("sub_notification"), HasPublicId, HasModifiers {
    override val publicId = nanoId("public_id", NANO_ID_MAX_LENGTH)
    override val createdAt = createdAt()
    override val updatedAt = updatedAt()

    val notificationId = ulong("notification_id").references(NotificationTable.id).index()
    val methodId = ulong("notification_method_id").references(NotificationMethodTable.id).index()
...
fun SubNotificationTable.rowToSubNotificationRecord(row: ResultRow): SubNotificationRecord =
