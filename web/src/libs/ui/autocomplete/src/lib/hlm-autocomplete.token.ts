...
const HlmAutocompleteConfigToken = new InjectionToken<HlmAutocompleteConfig<unknown, unknown>>(
...
export function injectHlmAutocompleteConfig<T, V = T>(): HlmAutocompleteConfig<T, V> {
  return (
    (inject(HlmAutocompleteConfigToken, {optional: true}) as HlmAutocompleteConfig<T, V> | null) ??
    getDefaultConfig()
  );
}
